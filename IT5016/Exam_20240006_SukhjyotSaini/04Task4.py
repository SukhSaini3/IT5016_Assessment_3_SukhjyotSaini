"""
Exam Task 4: Program to display staff information and requisition total alongside status.
...
def staff_info(requisition_id):
    ...
    requisition_id = str(unique_id) #The variable 'requisition_id' will read the incremented value as its input from 'unique_id' variable.
...
def display_requisitions(date, staff_id, requisition_id, staff_name, total_value, status):
    ...
    if status == "Approved":
        print(f"Approval Reference Number:{staff_id}{requisition_id[2:]}")
    elif status == "Pending":
...
def main():
    ...
    display_requisitions(date,staff_id, requisition_id, staff_name, total_value, status)
