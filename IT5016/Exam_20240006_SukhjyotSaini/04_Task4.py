...
def display_requisitions(date, staff_id, requisition_id, staff_name, total_value, status,requisition_id_str):
    ...
    print(f"Approval Reference Number:{staff_id}{requisition_id_str[2:]}")
...
def main():
    ...
    if status == "Approved": #Approval Reference Number will only be generated if the status is approved.
        ...
        requisition_id_str = str(requisition_id)

    else:
        print("")

    display_requisitions(date,staff_id, requisition_id, staff_name, total_value, status, requisition_id_str)
